...
class ComputeLoss:
    ...
    def __call__(self, p, targets, img_size=None):  # predictions, targets
        lcls = torch.zeros(1, device=self.device)  # class loss
        lbox = torch.zeros(1, device=self.device)  # box loss
        lobj = torch.zeros(1, device=self.device)  # object loss
        tcls, tbox, indices, anchors = self.build_targets(p, targets, img_size)  # targets
    ...
    def build_targets(self, p, targets, img_size=None):
        ...
        x_o = 2000  # Moves the center point of function to x_o (b)
        ...
        for i in range(self.nl):
            ...
            if nt:
                ...
                small_targets_min = torch.sum(
                    ...
                    torch.max(r, 1 / r).min(2)[0]) / num_targets_small
